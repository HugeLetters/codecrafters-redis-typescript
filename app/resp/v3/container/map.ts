...
const MapRegex = regex(
	`^${MapPrefix}(?<size>${RegexUtils.Digit}+)${CRLF}(?<entries>.*)$`,
	"s",
);
...
function decodeLeftoverMapSize(input: string, ast: SchemaAST.AST) {
	const decodeResult = Effect.gen(function* () {
		...
		const { size: rawSize, entries } = result.groups;
...
type RespMap = Schema.Schema<RespMapValue, string>;
